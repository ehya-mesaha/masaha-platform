...
export default function ProfileSettingsClient({ user }: { user: UserData }) {
  ...
  async function saveProfile() {
    ...
    try {
      ...
      setMessage({ type: 'ok', text: 'تم تحديث البيانات بنجاح' })
    } catch (err) {
      setMessage({ type: 'err', text: err instanceof Error ? err.message : 'حدث خطأ في الاتصال' })
    } finally {
      setSaving(false)
    }
  }
  ...
  async function uploadAvatar(file: File) {
    ...
    try {
      ...
      setMessage({ type: 'ok', text: 'تم تحديث صورة الملف الشخصي' })
    } catch (err) {
      setMessage({ type: 'err', text: err instanceof Error ? err.message : 'حدث خطأ أثناء رفع الصورة' })
    } finally {
  ...
  async function deleteAvatar() {
    ...
    try {
      ...
      setAvatarUrl('')
      setMessage({ type: 'ok', text: 'تم حذف صورة الملف الشخصي' })
    } catch (err) {
      setMessage({ type: 'err', text: err instanceof Error ? err.message : 'تعذر حذف الصورة' })
    } finally {
      setAvatarSaving(false)
    }
  }
