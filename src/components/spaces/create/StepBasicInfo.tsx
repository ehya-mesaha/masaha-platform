...
export default function StepBasicInfo({ form, update, types, categoriesLoading, categoriesError }: StepProps) {
  ...
  return (
    <div>
      <h2 className="font-display text-xl font-extrabold text-[#1B1B1B] mb-1">المعلومات الأساسية للمساحة</h2>
      <p className="text-[#5F6764] text-sm mb-6">أدخل التفاصيل الأولية لمساحتك لتساعد طالبي المساحات في فهم طبيعتها واستخداماتها.</p>

      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-[#3F4B47] mb-1.5">اسم المساحة <Required /></label>
          <input
            value={form.name}
        ...
        <div className="rounded-2xl border border-[#D8D1C7] bg-[#F5F1E8] p-4">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <label className="block text-sm font-extrabold text-[#0E3B34]">عدد القاعات المماثلة <Required /></label>
              <p className="mt-1 text-xs leading-6 text-[#5F6764]">سننشئ وحدة مستقلة لكل قاعة لإدارة التوفر ومنع تعارض الحجوزات.</p>
            </div>
            <div className="flex w-full items-center overflow-hidden rounded-xl border border-[#D9D1C2] bg-white sm:w-44" dir="ltr">
              <button
                ...
                className="grid h-11 w-12 place-items-center text-xl font-bold text-[#0E3B34] hover:bg-[#F5F1E8]"
              ...
              <input
                ...
                className="h-11 min-w-0 flex-1 border-x border-[#D8D1C7] text-center font-extrabold outline-none"
              />
              <button
                type="button"
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {Array.from({ length: Math.min(6, Number(form.identicalUnitsCount) || 1) }, (_, index) => (
              <span key={index} className="rounded-full bg-white px-3 py-1 text-[11px] font-bold text-[#0E3B34]">قاعة {101 + index}</span>
            ))}
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label className="block text-sm font-medium text-[#3F4B47] mb-1.5">تصنيف المساحة <Required /></label>
            <select
              value={form.typeId}
          ...
          <div>
            <label className="block text-sm font-medium text-[#3F4B47] mb-1.5">السعة الاستيعابية (أشخاص) <Required /></label>
            <div className="flex items-center border border-[#D8D1C7] rounded-lg overflow-hidden">
              <button
                type="button"
                onClick={() => update('capacity', String(Math.max(1, Number(form.capacity || 1) - 1)))}
                className="px-3 py-2.5 text-[#3F4B47] hover:bg-[#F5F1E8] transition-colors text-lg"
              >
                −
              </button>
              <input
                ...
                className="flex-1 text-center py-2.5 text-sm focus:outline-none border-x border-[#D8D1C7] [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                min={1}
                placeholder="12"
              />
              <button
        ...
        <div>
          <label className="block text-sm font-medium text-[#3F4B47] mb-1.5">وصف المساحة <Required /></label>
          <textarea
