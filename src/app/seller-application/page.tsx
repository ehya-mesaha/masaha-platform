    ...
    <div className="min-h-screen bg-[#F7F3EB]">
      ...
      <main className="mx-auto max-w-5xl px-4 py-10">
        <header className="page-hero mb-7 p-8">
          <p className="mb-2 text-xs font-bold text-[#D8B455]">انضم إلى شبكة إحياء مساحة</p>
        ...
        <form onSubmit={submit} className="space-y-5">
          ...
          <Section title="2. بيانات الجهة">
            ...
            <label className="mt-4 flex items-center gap-3 rounded-xl bg-[#F7F3EB] p-4 text-sm font-bold text-[#344139]">
          ...
          <Section title="3. المستندات">
            <p className="mb-4 text-sm text-[#6B7566]">PDF أو صورة واضحة. تحفظ المستندات للمراجعة الإدارية فقط.</p>
          ...
          <section className="premium-card p-6">
            <label className="flex items-start gap-3 text-sm leading-7 text-[#344139]"><input required className="mt-2" type="checkbox" checked={form.consentAccepted} onChange={(event) => setForm({ ...form, consentAccepted: event.target.checked })} /><span>أقر بصحة البيانات والمستندات، وأوافق على أن تتواصل معي إحياء مساحة لاستكمال المراجعة وعقد الوساطة.</span></label>
            {error && <p className="mt-4 rounded-xl bg-red-50 px-4 py-3 text-sm font-bold text-red-700">{error}</p>}
            <button disabled={loading || uploading !== null} className="mt-5 w-full rounded-xl bg-[#1B3A2D] px-5 py-3 font-bold text-white disabled:opacity-60">{loading ? 'جاري إرسال الطلب...' : 'إرسال طلب الانضمام'}</button>
            <p className="mt-4 text-center text-xs text-[#6B7566]">لديك حساب؟ <Link href="/auth/login" className="font-bold text-[#1B3A2D]">تسجيل الدخول</Link></p>
          </section>
        </form>
      </main>
    </div>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) { return <section className="premium-card p-6"><h2 className="mb-5 text-lg font-extrabold text-[#14201A]">{title}</h2>{children}</section> }
function Field({ label, value, set, type = 'text', dir, required = true }: { label: string; value: string; set: (value: string) => void; type?: string; dir?: 'ltr'; required?: boolean }) {
  return <label className="block"><span className="mb-1.5 block text-sm font-bold text-[#344139]">{label}</span><input required={required} type={type} dir={dir} value={value} onChange={(event) => set(event.target.value)} className="w-full rounded-xl border border-[#DDD6C8] px-4 py-3 outline-none focus:border-[#1B3A2D]" /></label>
}
function Upload({ label, type, required, uploaded, uploading, onFile }: { label: string; type: DocType; required: boolean; uploaded: boolean; uploading: DocType | null; onFile: (file: File, type: DocType) => void }) {
  return <label className={`cursor-pointer rounded-2xl border p-4 ${uploaded ? 'border-green-300 bg-green-50' : 'border-dashed border-[#CFC6B6] bg-[#FBFAF7]'}`}><span className="block text-sm font-bold text-[#14201A]">{label}{required ? ' *' : ''}</span><span className="mt-1 block text-xs text-[#6B7566]">{uploading === type ? 'جاري الرفع...' : uploaded ? 'تم الرفع — اضغط للاستبدال' : 'اضغط لاختيار الملف'}</span><input required={required && !uploaded} type="file" accept="image/*,.pdf" className="sr-only" onChange={(event) => event.target.files?.[0] && onFile(event.target.files[0], type)} /></label>
}
